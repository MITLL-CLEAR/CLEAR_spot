...
class ImageInsideOpenGL():
    ...
    def __init__(self, numpy_array):
        ...
        with self.manage_bind():
            # If your numpy array includes an alpha channel, use GL_RGBA instead of GL_RGB
            internal_format = GL_RGBA if numpy_array.shape[2] == 4 else GL_RGB
            data_format = GL_RGBA if numpy_array.shape[2] == 4 else GL_RGB
            glTexImage2D(GL_TEXTURE_2D, 0, internal_format, numpy_array.shape[1], numpy_array.shape[0], 0, 
                         data_format, GL_UNSIGNED_BYTE, numpy_array)
...
def crop_image(image, top_cutoff, bottom_cutoff):
        """
        Crops the top and bottom of the image.

        :param top_cutoff: The amount to cut off from the top of the image.
        :param bottom_cutoff: The amount to cut off from the bottom of the image.
        :return: Cropped image.
        """
        if image is not None:
            img_height = image.shape[0]
            # Ensure cutoffs are within the image dimensions
            top_cutoff = min(top_cutoff, img_height)
            bottom_cutoff = min(bottom_cutoff, img_height - top_cutoff)

            # Perform the cropping
            cropped_image = image[top_cutoff:img_height - bottom_cutoff, :]
...
class ImageGenerator():
    def __init__(self, imageClient) -> None:
        self.imageClient = imageClient
        self.image = None

    def generateImage(self):
        # Retrieve the dimensions of the current OpenGL viewport
        viewport = glGetIntegerv(GL_VIEWPORT)
        x, y, width, height = viewport
        # Read the pixel data from the OpenGL buffer
        pixels = glReadPixels(x, y, width, height, GL_RGB, GL_UNSIGNED_BYTE)
        # Convert the pixel data to an Image in PIL
        image = Image.frombytes("RGB", (width, height), pixels)
        # Flip the image vertically as OpenGL origin is bottom-left and PIL's is top-left
        image = image.transpose(Image.FLIP_TOP_BOTTOM)
        # Save the image to a file
        # image.save("test.jpg", 'JPEG')

        opencv_image = numpy.array(image)[ :, :, ::-1].copy()

        self.image = crop_image(opencv_image, 130, 85)
    ...
    def stitch(self):
        ...
        with open('SpotController/CameraHandler/renderingImage/shader_vert.glsl', 'r') as file:
            vert_shader = file.read()
        with open('SpotController/CameraHandler/renderingImage/shader_frag.glsl', 'r') as file:
        ...
        requests = [
            build_image_request(source, pixel_format=pixel_format_enum, quality_percent=100)
        ...
        while running:
            ...
            if images_future is None:
                images_future = self.imageClient.get_image_async(requests, timeout=5.0)
...
def startRenderer(imgCli):
    renderObject = ImageGenerator(imgCli) 
    renderThread = threading.Thread(target=renderObject.stitch, daemon=True)
    renderThread.start()
    return renderObject
